...
def tool_bulk_dns():
    ...
    print("1. Resolve hostnames to IPs")
    print("2. Resolve IPs to hostnames")
    ...
    if mode == "1":
        ...
        print("Enter IPs separated by comma (e.g., 8.8.8.8, 1.1.1.1):")
        raw = input("> ").strip()
        ips = [ip.strip() for ip in raw.split(',')]
       
        print(f"\n{C_BOLD}{'IP Address':<20} {'Resolved Hostname':<30}{C_RESET}")
        print("-" * 50)
        for ip in ips:
            try:
                host = socket.gethostbyaddr(ip)[0]
                print(f"{ip:<20} {C_GREEN}{host:<30}{C_RESET}")
            except socket.herror:
                print(f"{ip:<20} {C_RED}Resolution Failed{C_RESET}")
   
    else:
        print(f"{C_RED}Invalid option. Please choose 1 or 2.{C_RESET}")
